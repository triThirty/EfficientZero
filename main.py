...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--save_video", action="store_true", default=False, help="save video in test."
    )
    parser.add_argument(
        "--force",
        action="store_true",
    ...
    parser.add_argument(
        "--p_mcts_num", type=int, default=4, help="number of parallel mcts"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--use_max_priority", action="store_true", default=False, help="max priority"
    )
    parser.add_argument(
        "--test_episodes",
        type=int,
    ...
    parser.add_argument(
        "--load_model", action="store_true", default=False, help="choose to load model"
    )
    parser.add_argument(
        "--model_path",
        type=str,
    ...
    assert (
        args.revisit_policy_search_rate is None
        or 0 <= args.revisit_policy_search_rate <= 1
    ...
    try:
        if args.opr == "train":
            ...
            if args.model_path is None:
                ...
                model_path = args.model_path
            assert os.path.exists(model_path), "model not found at {}".format(
                model_path
            )
